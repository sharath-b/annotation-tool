...
class ApiController extends BaseController {
  async index(additionalCriteria = {}) {
    const { service } = this;
    const { criteria, options } = await this.checkAndBuildQuery(this.ctx.query);
    const page = await service.paginationService.findAll(this.model, { ...criteria, ...additionalCriteria }, options);
    this.validation = null;
    return this.success(page);
  }

  async checkAndBuildQuery(query) {
    const options = {
